...
for (var i = 0; i < colNum; i++) {
  for (var j = 0; j < rowNum; j++) {
    $(".game").append(
      '<div id="' + counter + '" class="tile tile_' + counter + '"></div>'
    );
    counter++;
  }
}
...
$(".tile").each(function () {
  ...
  if ($this.hasClass("top-border") && $this.hasClass("left-border")) {
    ...
    $this.hasClass("right-border") &&
    $this.hasClass("bottom-border")
  ) {
    $this
      .removeClass("right-border bottom-border")
      .addClass("bottom-right-corner");
  } else if ($this.hasClass("left-border") && $this.hasClass("bottom-border")) {
    $this
      .removeClass("left-border bottom-border")
      .addClass("bottom-left-corner");
  }
});
...
$.fn.findTargetTiles = function () {
  ...
  if (!$this.hasClass("bomb")) {
    if ($this.hasClass("top-left-corner")) {
      ...
      var topTile = ref - col;
      targetTiles.push(ref + 1, ref - 1, topTile, topTile + 1, topTile - 1);
    } else {
      var rightTile = ref + 1,
        leftTile = ref - 1,
        bottomTile = ref + col,
...
//   $( "#divqualquer" ).hide( "slow", function() {
...
//   }, mouseenter: function() {
//     $( this ).addClass( "mouse-entrou" );
//   }, mouseleave: function() {
//     $( this ).removeClass( "mouse-saiu" );
//   }
// });

// $( "div" ).on( "click", function( event, pessoa ) {
//   alert( "Olá, " + pessoa.nome );
// });
// $( "div" ).trigger( "click", { nome: "João" } );
